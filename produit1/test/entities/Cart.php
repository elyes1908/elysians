...
class Cart {
    ...
    public function insert($item = array()){
        if(!is_array($item) OR count($item) === 0){
            ...
            if(!isset($item['reference'], $item['nom'], $item['prix'], $item['qty'])){
                return FALSE;
            }else{
                /*
                 * Insert Item
    ...
    public function update($item = array()){
        if (!is_array($item) OR count($item) === 0){
            ...
            if (!isset($item['rowid'], $this->cart_contents[$item['rowid']])){
                ...
                foreach(array_diff($keys, array('reference', 'nom')) as $key){
    ...
    protected function save_cart(){
        ...
        foreach ($this->cart_contents as $key => $val){
            ...
            $this->cart_contents['cart_total'] += ($val['prix'] * $val['qty']);
            $this->cart_contents['total_items'] += $val['qty'];
            $this->cart_contents[$key]['subtotal'] = ($this->cart_contents[$key]['prix'] * $this->cart_contents[$key]['qty']);
        }
