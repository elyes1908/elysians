...
function afficherEmploye ($employe){
	...
	function ajouterEmploye($employe){
		$sql="insert into employe (cin,nom,prenom,image,nbHeures,tarifHoraire) values (:cin, :nom,:prenom,:image,:nbH,:tarifH)";
	...
	function modifierEmploye($employe,$cin){
		$sql="UPDATE employe SET cin=:cinn, nom=:nom,prenom=:prenom,image=:image,nbHeures=:nbH,tarifHoraire=:tarifH WHERE cin=:cin";
...
try{		
        ...
        $nb=$employe->getNbHeures();
        $tarif=$employe->getTarifHoraire();
		$datas = array(':cinn'=>$cinn, ':cin'=>$cin, ':nom'=>$nom,':prenom'=>$prenom,':image'=>$image,':nbH'=>$nb,':tarifH'=>$tarif);
